...
var sfillEqual = tryRequire( resolve( __dirname, './../lib/sfill_equal.native.js' ) );
var opts = {
	'skip': ( sfillEqual instanceof Error )
};
...
tape( 'the function has an arity of 5', opts, function test( t ) {
	t.strictEqual( sfillEqual.length, 5, 'has expected arity' );
	t.end();
});

tape( 'the function replaces values equal to the provided `searchElement` in a strided array', opts, function test( t ) {
	var expected;
	var x;
	...
	sfillEqual( x.length, 1.0, 5.0, x, 1 );
...
tape( 'the function returns a reference to the input array', opts, function test( t ) {
	...
	x = new Float32Array( [ 0.0, 1.0, 3.0, 1.0, 5.0 ] );
	out = sfillEqual( x.length, 1.0, 5.0, x, 1 );
